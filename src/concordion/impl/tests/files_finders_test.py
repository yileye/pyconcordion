...
class FolderRunnerTest(unittest.TestCase):
    ...
    def test_creation_needs_folder_name(self):
        "Folder Finder - needs a folder name"
        try:
            FolderTestFinder(["Exec_name"])
        except SystemExit, e:
            self.assertEquals(1, e.code)
            
    def test_creation_saves_folder_name(self):
        "Folder Finder - saves the given folder name"
        runner = FolderTestFinder(["Exec_name", "folder_name"])
    ...
    def test_can_find_py_files_directly_under_folder(self):
        ...
        runner = FolderTestFinder(["Exec_name", "example_folder"])
